  ...
  export const SwaggerDocsView: React.FC = () => {
    ...
    const endpoints = [
      ...
      { method: 'POST', path: '/api/assignments', desc: 'Create assignment (Teacher)', auth: true, body: '{\n  "title": "Calculus Integration",\n  "description": "Solve problems 1-10",\n  "classId": "cls-101",\n  "subjectId": "sbj-math101",\n  "deadline": "2026-08-30T23:59:59.000Z",\n  "maxMarks": 100,\n  "status": "Published"\n}' },
      { method: 'PATCH', path: '/api/assignments/asg-bf7d2aba-9249-4dab-aa31-ec33555b1896/status', desc: 'Update assignment status (Draft/Published/Closed)', auth: true, body: '{\n  "status": "Closed"\n}' },
    ...
    //     e => `${e.method} ${e.path}` === activeEndpoint
    //   );
        ...
        if (
          ...
          try {
            ...
            setTestResult({
              status: 400,
              ok: false,
              error: 'Invalid JSON payload. Please check your JSON syntax.',
            });
    ...
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        ...
        <button
          type="button"
          onClick={() => setRequestBody(ep.body || '')}
      ...
      <textarea
        ...
        spellCheck={false}
        className="w-full min-h-[250px] p-4 bg-slate-900 text-emerald-400 rounded-xl text-xs font-mono border border-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-y"
        placeholder="Enter JSON payload..."
      />

      <p className="text-[11px] text-slate-400">
        Edit the JSON payload above and click <strong>Test Request</strong> to
        send it.
      </p>
    </div>
  )}
